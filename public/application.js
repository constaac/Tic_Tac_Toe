...
/***/ (function(module, exports, __webpack_require__) {
	...
	var resetCurrentStats = function resetCurrentStats() {
	  currentTurn = '';
	  currentGame = [];
	  currentGameID = {};
	...
	var setTurnIndicator = function setTurnIndicator() {
	...
	// turn calculator
	var calcTurn = function calcTurn(response) {
	  var xcounter = 0;
	  var ocounter = 0;
	  response.game.cells.forEach(function (item) {
	    if (item === 'x') {
	      xcounter = xcounter + 1;
	    } else if (item === 'o') {
	      ocounter = ocounter + 1;
	  ...
	  var totalCount = xcounter + ocounter;
	  if (totalCount % 2 === 0) {
	    currentTurn = 'x';
	  } else {
	    currentTurn = 'o';
	...
	var overChecker = function overChecker() {
	  var movesTaken = 0;
	...
	var calcCurrentTurnCounter = function calcCurrentTurnCounter(data) {
	  var tempCounter = 0;
	  data.game.cells.forEach(function (element) {
	    if (element !== '') {
	      tempCounter += 1;
	    }
	  });
	...
	var restartOnClicks = function restartOnClicks(x) {
	  if (x === 0) {
	...
	var restartAllOnClicks = function restartAllOnClicks(response) {
	  $('.div0').on('click', onTopLeft);
	  $('.div1').on('click', onTopMiddle);
	  $('.div2').on('click', onTopRight);
	  $('.div3').on('click', onMiddleLeft);
	...
	var onTopLeft = function onTopLeft() {
	  changeSpace(0);
	};
	var onTopMiddle = function onTopMiddle() {
	  changeSpace(1);
	};
	var onTopRight = function onTopRight() {
	  changeSpace(2);
	};
	var onMiddleLeft = function onMiddleLeft() {
	  changeSpace(3);
	};
	var onMiddleMiddle = function onMiddleMiddle() {
	  changeSpace(4);
	};
	var onMiddleRight = function onMiddleRight() {
	  changeSpace(5);
	};
	var onBottomLeft = function onBottomLeft() {
	  changeSpace(6);
	};
	var onBottomMiddle = function onBottomMiddle() {
	  changeSpace(7);
	};
	var onBottomRight = function onBottomRight() {
	  changeSpace(8);
	};

	var changeSpace = function changeSpace(cellindex) {
	  ...
	  if ($('#' + cellindex).text() === '' && $.active === 0) {
	    ...
	    if (currentTurn === 'x') {
	      api.updateGameAPI(game).then(function (response) {
	        ...
	        return response;
	      }).then(updateGame).then(function (response) {
	...
	var changeColor = function changeColor(index, color) {
	  $('#' + index).css('color', color);
	};

	var resetBoardColors = function resetBoardColors() {
	  for (var i = 0; i < 9; i++) {
	    changeColor(i, 'black');
	  }
	};

	var setToUpper = function setToUpper(data) {
	  for (var i = 0; i < 9; i++) {
	    $('#' + i).text(data[i].toUpperCase());
	  }
	};

	var updateGame = function updateGame(response) {
	...
	var winCheck = function winCheck(x) {
	...
	module.exports = {
	  ...
	  overChecker: overChecker,
	  changeSpace: changeSpace,
	  resetCurrentStats: resetCurrentStats,
	  calcCurrentTurnCounter: calcCurrentTurnCounter,
	  setTurnIndicator: setTurnIndicator,
	  winCheck: winCheck,
	  resetBoardColors: resetBoardColors,
	  restartAllOnClicks: restartAllOnClicks
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameHistory = function gameHistory(x) {
	  if (x === '0') {
	    return $.ajax({
	      headers: {
	        'Authorization': 'Token token=' + store.userToken
	...
	var updateGameAPI = function updateGameAPI(gamedata) {
	  return $.ajax({
	    headers: {
	      'Authorization': 'Token token=' + store.userToken
	    },
	...
	var startLoadGame = function startLoadGame(data) {
	  return $.ajax({
	    headers: {
	      'Authorization': 'Token token=' + store.userToken
	    },
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onCreateGame = function onCreateGame() {
	  gamelogic.resetCurrentStats();
	  api.createGame().then(ui.createGameSuccess).then(gamelogic.updateGame).then(gamelogic.restartAllOnClicks).catch(ui.createGameFailure);
	};

	var onGameHistory = function onGameHistory() {
	  var query = $('input[name="history-query"]:checked').val();
	  api.gameHistory(query).then(ui.gameHistorySuccess).catch(ui.gameHistoryFailure);
	};

	var onStatistics = function onStatistics() {
	  api.gameStatistics().then(ui.statisticsSuccess).catch(ui.statisticsFailure);
	};
	...
	var onLoadGame = function onLoadGame(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  if (data.game.id === '') {
	  ...
	  api.startLoadGame(data).then(ui.loadGameSuccess).then(gamelogic.restartAllOnClicks).then(function (response) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameHistoryFailure = function gameHistoryFailure(response) {
	...
	var statisticsSuccess = function statisticsSuccess(response) {
	  var arrayofgames = [];
	  ...
	  var dcounter = 0;
	  var overcounter = 0;
	  var notovercounter = 0;
	  arrayofgames.forEach(function (response) {
	    if (response.over) {
	      overcounter += 1;
	    } else {
	      notovercounter += 1;
	  ...
	  var totalwins = xcounter + ocounter;
	  var wdratioraw = (xcounter + ocounter) / dcounter;
	  var xoratioraw = xcounter / ocounter;
	  var wdratio = wdratioraw;
	  if (isNaN(wdratio)) {
	    wdratio = 0;
	  }
	  var xoratio = xoratioraw;
	  if (isNaN(xoratio)) {
	    xoratio = 0;
	  }
	...
	var statisticsFailure = function statisticsFailure() {
	...
	module.exports = {
	  ...
	  statisticsFailure: statisticsFailure,
	  FTWRIn: FTWRIn
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
